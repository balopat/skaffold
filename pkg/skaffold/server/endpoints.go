...
func (s *server) Sync(context.Context, *empty.Empty) (*proto.ApiResponse, error) {
	if s.syncTrigger == nil {
		return nil, status.Errorf(codes.FailedPrecondition, "sync deploy trigger is not enabled")
	}
	if len(s.syncTrigger) > 0 {
		return &proto.ApiResponse{
			Response: "sync trigger already queued",
		}, nil
	}
	s.syncTrigger <- true
	return &proto.ApiResponse{
		Response: "sync trigger received",
	}, nil
}
